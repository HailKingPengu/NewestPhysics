...
namespace Volatile
{
    public sealed class VoltPolygon : VoltShape
    {
        ...
        internal void InitializeFromWorldVertices(
          ...
          float restitution)
        {
            base.Initialize(density, friction, restitution, frame);
        ...
        public void Update()
        {
            rotation = (worldVertices[1] - worldVertices[0]).angleInDeg;
        }
        #endregion
          ...
          ref Axis[] destination)
        {
            ...
            {
                Vec2 u = vertices[i];
                Vec2 v = vertices[(i + 1) % count];
                Vec2 normal = (v - u).Left().normalized;
                destination[i] = new Axis(normal, Vec2.Dot(normal, u));
            }
        }
        ...
        internal Vec2[] bodyVertices;
        internal Axis[] bodyAxes;
        internal int countBody;
        #endregion

        public VoltPolygon() : base("../../Assets/crate.png")
        {
            this.Reset();
        }
          ...
          Vec2 bodySpacePoint)
        {
            for (int i = 0; i < this.countBody; i++)
            {
                ...
                if (Vec2.Dot(axis.Normal, bodySpacePoint) > axis.Width)
                    return false;
            }
            return true;
        }
          ...
          float radius)
        {
            ...
            Vec2 a = this.bodyVertices[foundIndex];
            Vec2 b = this.bodyVertices[(foundIndex + 1) % numVertices];
          ...
          ref VoltRayResult result)
        {
            ...
            for (int i = 0; i < this.countBody; i++)
            {
                ...
                float proj =
                  Vec2.Dot(curAxis.Normal, bodySpaceRay.origin) - curAxis.Width;
                ...
                float slope = Vec2.Dot(-curAxis.Normal, bodySpaceRay.direction);
        ...
        /// </summary>
        internal void GetEdge(int indexFirst, out Vec2 a, out Vec2 b)
        {
        ...
        internal bool ContainsPoint(
          Vec2 worldSpacePoint)
        {
            for (int i = 0; i < this.countWorld; i++)
            {
                Axis axis = this.worldAxes[i];
                if (Vec2.Dot(axis.Normal, worldSpacePoint) > axis.Width)
                    return false;
            }
            return true;
        }
        ...
        internal bool ContainsPointPartial(
          Vec2 worldSpacePoint,
          Vec2 worldSpaceNormal)
        {
            foreach (Axis axis in this.worldAxes)
                if (Vec2.Dot(axis.Normal, worldSpaceNormal) >= 0.0f &&
                    Vec2.Dot(axis.Normal, worldSpacePoint) > axis.Width)
                    return false;
            return true;
        }
        #endregion
        ...
        private void UpdateArrays(int length)
        {
            ...
            {
                this.worldVertices = new Vec2[length];
            ...
            {
                this.bodyVertices = new Vec2[length];
        ...
        private float ComputeArea()
        {
            ...
            for (int i = 0; i < this.countBody; i++)
            {
                Vec2 v = this.bodyVertices[i];
                Vec2 u = this.bodyVertices[(i + 1) % this.countBody];
                Vec2 w = this.bodyVertices[(i + 2) % this.countBody];
        ...
        private float ComputeInertia()
        {
            ...
            for (int i = 0; i < this.countBody; i++)
            {
                ...
                float b = v.lengthSquared + u.lengthSquared + Vec2.Dot(v, u);
                s1 += a * b;
                s2 += a;
            }
          ...
          ref VoltRayResult result)
        {
            ...
            Vec2 v3 = bodySpaceRay.direction.Left();
            ...
            for (int i = 0; i < this.countBody; i++)
            {
                ...
                Vec2 extension = curAxis.Normal * radius;
                Vec2 a = this.bodyVertices[i] + extension;
                Vec2 b = this.bodyVertices[(i + 1) % this.countBody] + extension;
                ...
                if (Vec2.Dot(curAxis.Normal, bodySpaceRay.direction) >= 0.0f)
                    continue;

                // See: 
                // https://rootllama.wordpress.com/2014/06/20/ray-line-segment-intersection-test-in-2d/
                Vec2 v1 = bodySpaceRay.origin - a;
                Vec2 v2 = b - a;

                float denominator = Vec2.Dot(v2, v3);
                float t1 = VoltMath.Cross(v2, v1) / denominator;
                float t2 = Vec2.Dot(v1, v3) / denominator;
      ...
      float normalLength)
    {
      ...
      for (int i = 0; i < this.countWorld; i++)
      {
        Vec2 u = this.worldVertices[i];
        Vec2 v = this.worldVertices[(i + 1) % this.countWorld];
        Vec2 n = worldAxes[i].Normal;

        Vec2 delta = v - u;
        Vec2 midPoint = u + (delta * 0.5f);
