...
namespace Volatile
{
  internal static class Collision
  {
    ...
    {
      Vec2 toOrigin = shapeOrigin - ray.origin;
      ...
      float slope = Vec2.Dot(toOrigin, ray.direction);
      if (slope < 0)
        return false;

      float sqrSlope = slope * slope;
      float d = sqrRadius + sqrSlope - Vec2.Dot(toOrigin, toOrigin);
      if (d < 0)
        return false;
      ...
      Vec2 normal = (dist * ray.direction - toOrigin).normalized;
      ...
      out float minDistance)
    {
      ...
      {
        float dot = Vec2.Dot(axes[i].Normal, point);
      ...
      out float penetration)
    {
      ...
      for (int i = 0; i < poly.countWorld; i++)
      {
        ...
        float dot = Vec2.Dot(axis.Normal, origin);
    ...
    private static Manifold TestCircles(
      ...
      VoltCircle shapeA,
      VoltShape shapeB,
      Vec2 overrideBCenter, // For testing vertices in circles
      float overrideBRadius)
    {
      Vec2 r = overrideBCenter - shapeA.worldSpaceOrigin;
    ...
    private static bool FindMinSepAxis(
      ...
      out Axis axis)
    {
      axis = new Axis(Vec2.Zero, float.NegativeInfinity);
      ...
      for (int i = 0; i < poly1.countWorld; i++)
      {
        ...
        {
          Vec2 v = poly2.worldVertices[j];
          min = Mathf.Min(min, Vec2.Dot(a.Normal, v));
        }
        min -= a.Width;
      ...
      Manifold manifold)
    {
      ...
      for (int i = 0; i < poly1.countWorld; i++)
      {
        Vec2 vertex = poly1.worldVertices[i];
      ...
      {
        Vec2 vertex = poly2.worldVertices[i];
